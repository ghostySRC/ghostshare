(function (root) {
  ...
  class GhostyUI extends EventTarget {
    ...
    build(layouts) {
      ...
      panel.innerHTML = `
        ...
        <div class="gmp-body"><div class="gmp-space gmp-card" style="margin-bottom:12px"><div class="gmp-row"><span id="gmp-status-dot" class="gmp-status-dot"></span><span id="gmp-status-text">Offline</span></div><div id="gmp-ping">-- ms</div></div>
          ...
          <div id="gmp-browse-view" class="gmp-view"><div class="gmp-space" style="margin-bottom:12px"><div><h2 style="margin:0">Public rooms</h2><div class="gmp-muted">Live rooms hosted by other GMP players</div></div><button id="gmp-refresh-rooms" class="gmp-button">Refresh</button></div><div id="gmp-public-room-list" class="gmp-card"></div></div>
    ...
    setRaceState(state={}) { this.refs.raceClock.textContent=state.label||"READY"; if(state.message)this.refs.raceStatus.textContent=state.message; }
    ...
    destroy() { this.root?.remove();this.panel?.remove();this.refs.backdrop?.remove();this.refs.toast?.remove(); }
  }
